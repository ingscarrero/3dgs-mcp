...
| Container | Privilege escalation | Runs as a non-root user; no published ports; single bind mount. |

## Known limitations

- The key is passed to the container via environment variables and is
  therefore visible to the same OS user through `podman inspect`. This is
  accepted for a single-user workstation.
