...
| `get_project` | `projectId` | `{ project, latestRun \| null }` — full `meta.json` and `run.json` | None |
...
| `list_splats` | — | `{ splats: [{ projectId, name, splats: [paths] }], count }` | None |
| `open_studio` | `path?` (relative route, default `/`) | Confirmation text with the URL | Opens the browser in host mode; returns the URL in container mode |
...
- **Container.** Non-root user, no published ports, single bind mount.
